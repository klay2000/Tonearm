...
// Both default to "off" — opt-in until users have a chance to review how
// merging affects their library.
export const mergeCaseDuplicates = writable(
  (localStorage.getItem('subsonic_merge_case_duplicates') ?? 'false') === 'true'
)
...
export const splitCollabAlbums = writable(
  (localStorage.getItem('subsonic_split_collab_albums') ?? 'false') === 'true'
)
